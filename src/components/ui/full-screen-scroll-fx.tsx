...
export const FullScreenScrollFX = forwardRef<HTMLDivElement, FullScreenFXProps>(
  (
    ...
    useLayoutEffect(() => {
      ...
      // initial bg states - ensure first image is visible
      // Force display of current section background immediately
      bgRefs.current.forEach((bg, i) => {
        if (bg) {
          // Ensure image is visible in the DOM first
    ...
    const goTo = (to: number, withScroll = true) => {
      ...
      if (withScroll && typeof window !== "undefined") {
        ...
        if (smoothScroll) {
          // Lenis integration would go here
    ...
    return (
      ...
      >
                      ...
                      <>
                        <img
                          ref={(el) => {
                            if (el) {
                              bgRefs.current[i] = el;
                              // Force immediate visibility for current section
                              if (i === index) {
                                el.style.opacity = '1';
                          ...
                          src={s.background}
                          alt={`AI Automation Background ${i + 1}`}
                          className="fx-bg-img"
                          loading={i === 0 ? "eager" : "lazy"}
                          onLoad={(e) => {
                            console.log(`Image ${i + 1} loaded successfully:`, s.background);
                          ...
                          onError={(e) => {
                            console.error(`Image ${i + 1} failed to load:`, s.background);
                            // Try fallback gradient instead of hiding
        ...
        <style>{`
          ...
          .fx-bg:first-child .fx-bg-img { opacity: 1 !important; visibility: visible !important; z-index: 10 !important; }
          .fx-bg-img {
          ...
          .fx-bg-overlay { position: absolute; inset: 0; background: var(--fx-overlay); z-index: 2; }
          ...
          .fx-featured { 
            ...
            max-width: 90%; word-wrap: break-word; white-space: nowrap;
          }
