...
const DotMaterial = shaderMaterial(
  ...
  /* glsl */ `
    ...
    void main() {
      ...
      float scaleInfluence = max(mouseInfluence * 1.2, circleAnimatedMask * 0.3);

      // Create dots with animated scale, influenced by mouse
      float dotSize = min(pow(circleMaskCenter, 2.0) * 0.3, 0.3);

      float sdfDot = sdfCircle(gridUv, dotSize * (1.0 + scaleInfluence * 1.0));

      float smoothDot = smoothstep(0.05, 0.0, sdfDot);

      float opacityInfluence = max(mouseInfluence * 120.0, circleAnimatedMask * 0.8);
