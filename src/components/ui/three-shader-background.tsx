...
const WaveShaderMaterial = shaderMaterial(
  ...
  `
    ...
    void main() {
      ...
      float wave1 = sin(st.x * 6.0 + uTime * 1.8) * 0.08;
      float wave2 = sin(st.y * 4.0 + uTime * 1.2) * 0.08;
      float wave3 = sin((st.x + st.y) * 3.0 + uTime * 2.5) * 0.04;
      ...
      float noiseValue = smoothNoise(st * 2.5 + uTime * 0.4);
      float fbmNoise = fbm(st * 1.8 + uTime * 0.25);
      float combinedNoise = mix(noiseValue, fbmNoise, 0.5);
      ...
      float glow = 1.0 - distance(st, vec2(0.5)) * 1.0;
      ...
      float brightness = 0.7 + sin(uTime * 0.6) * 0.1;
      color *= brightness;
      
      // Reduce opacity for background use
...
function ShaderPlane() {
  ...
  useFrame((state) => {
    if (materialRef.current) {
      ...
      materialRef.current.uMouse.lerp(new THREE.Vector2(mouse.current.x, mouse.current.y), 0.03)
...
export default function ThreeShaderBackground({ className, children }: ThreeShaderBackgroundProps) {
  return (
