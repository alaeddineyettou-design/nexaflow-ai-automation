...
export const HeroScrollVideo: React.FC<HeroScrollVideoProps> = ({
  ...
  subtitle = "Next-Gen Integration Platform",
  meta = "2025",
  credits = (
    <>
      <p>Powered by</p>
      <p>Advanced AI Systems</p>
    </>
  ),
  ...
  overlay = {
    ...
    paragraphs: [
      "Experience the power of AI-driven automation with n8n and Make.com integrations.",
      "Built with advanced 3D visualizations and real-time workflow orchestration.",
    ],
    extra: null,
  },
  ...
  const renderMedia = () => {
    if (mediaType === "image") {
      // Check if it's empty media (for AI visualization)
      if (!media || (typeof media === "string" && media === "")) {
        // Import and render AI visualization dynamically
        const AI3DVisualization = React.lazy(() => import("../AI3DVisualization"));
        return (
          <React.Suspense fallback={
            <div className="w-full h-full bg-gradient-radial from-orange-500/20 to-black flex items-center justify-center">
              <div className="text-orange-400">Loading AI Visualization...</div>
            </div>
          }>
            <AI3DVisualization className="w-full h-full" />
          </React.Suspense>
        );
      }
